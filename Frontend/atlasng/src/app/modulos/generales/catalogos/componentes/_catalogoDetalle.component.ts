...
export class CatalogoDetalleComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  public crearDtoConsulta(): Consulta {
    ...
    const consulta = new Consulta(this.entityBean, 'Y', this.mcampos.orderby, this.mfiltros, this.mfiltrosesp);
    this.addConsulta(consulta);
    return consulta;
  }

  public fijarFiltrosConsulta() {
    this.mcampos.orderby = 't.cdetalle';
    if (this.mfiltros.ccatalogo === 220) {
      this.mfiltrosesp.cdetalle = `not in ('21','22')`;
      if (this.mod !== '10') {
      //  this.mfiltrosesp.clegal = 'is null';
        this.mfiltrosesp.cdetalle = `not in ('21','22')`;
      }
    }

    if (this.mod === '27') {
      this.mcampos.orderby = 't.nombre';
    }
  }
