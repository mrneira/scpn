...
export class LovModuloTareasComponent extends BaseComponent implements OnInit {
  ...
  public crearDtoConsulta(): Consulta {
    const consulta = new Consulta(this.entityBean, 'Y', 't.pk', this.mfiltros, this.mfiltrosesp);
  ...
  showDialog(cmodulo: any = null) {
    if (cmodulo !== undefined && cmodulo !== null) {
      this.mfiltros.pk_cmodulo = cmodulo;
  ...
  llenarConsultaCatalogos(): void {
    const consultaIdioma = new Consulta('TgenModulo', 'Y', 't.nombre', {'activo': 1}, {});
  ...
  private manejaRespuestaCatalogos(resp: any) {
    const msgs = [];
    if (resp.cod === 'OK') {
      this.llenaListaCatalogo(this.lmodulo, resp.MODULOS);
    }
    this.lconsulta = [];
  }
