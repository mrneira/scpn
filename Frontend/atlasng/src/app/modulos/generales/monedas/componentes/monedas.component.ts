...
export class MonedasComponent extends BaseComponent implements OnInit, AfterViewInit {

  @ViewChild('formFiltros') formFiltros: NgForm;

  constructor(router: Router, dtoServicios: DtoServicios) {
    super(router, dtoServicios, 'TgenMoneda', 'MONEDA', false, false);
      ...
      return this.dtoServicios.ejecutarConsultaRest(rq).subscribe(
          resp => {
              if (resp.cod === 'OK') {
                ...
                for(let i in lista) {
                    let reg = lista[i];
                    pLista.push({ label: reg.nombre, value: reg.cmoneda});
                }
              } else {
