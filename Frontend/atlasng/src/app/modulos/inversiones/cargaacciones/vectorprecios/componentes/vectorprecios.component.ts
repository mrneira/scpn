...
export class VectorpreciosComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  public postCommit(resp: any) {
    ...
    if (resp.cod == "OK" && resp.msgusu == undefined && this.rqMantenimiento.cargaarchivo == 'read') {
      this.msgs = [];
      this.msgs.push({ severity: 'success', summary: 'CARGA DEL VECTOR DE PRECIOS EXITOSA', detail: '' });
  ...
  getBase64(inputValue: any): void {
    ...
    this.mcampos.ntipoarchivo= file.type;
    myReader.readAsDataURL(file);
  }

  propagateChange = (value: any) => {
    this.rqMantenimiento.archivo = value;
    this.rqMantenimiento.mdatos.ntipoarchivo = this.mcampos.ntipoarchivo;
