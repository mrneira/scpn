...
export class InternacionalComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  aprobarEtapa() {
    this.mcampos.msg = "INGRESE EL COMENTARIO PARA GENERAR EL COMPROBANTE SI SE REGISTRAN DIFERENCIAS DE PRECIOS";
  ...
  uploadHandler(event) {
    if (this.estaVacio(this.mcampos.fecha))
    {
      this.mostrarMensajeError("FECHA OBLIGATORIA");
      return;
    }
    ...
    let lfecha: number = (this.mcampos.fecha.getFullYear() * 10000) + ((this.mcampos.fecha.getMonth() + 1) * 100) + this.mcampos.fecha.getDate();

    this.rqMantenimiento.fecha = lfecha;
