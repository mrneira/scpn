...
export class RentafijaComponent extends BaseComponent
  ...
  ngOnInit() {
    ...
    if (sessionStorage.getItem('t') == "10" ||    //'PCCERO'
      ...
      if (lTran == "10" || lTran == "1010" || lTran == "3010") {
        this.inversionesServicios.pInstrumentocdetalle = "PCCERO";
      }
      else if (lTran == "11" || lTran == "1011" || lTran == "3011") {
      ...
      else if (lTran == "14" || lTran == "1014" || lTran == "3014") {
        this.inversionesServicios.pInstrumentocdetalle = "PA";
      }
      else if (lTran == "15" || lTran == "1015" || lTran == "3015") {
        this.inversionesServicios.pInstrumentocdetalle = "CDP";
      }
      else if (lTran == "16" || lTran == "1016" || lTran == "3016") {
        this.inversionesServicios.pInstrumentocdetalle = "BONO";
      }
      else if (lTran == "17" || lTran == "1017" || lTran == "3017") {
      ...
      else if (lTran == "20" || lTran == "1020" || lTran == "3020") {
        this.inversionesServicios.pInstrumentocdetalle = "FACCO";
      }
      else if (lTran == "21" || lTran == "1021" || lTran == "3021") {
        this.inversionesServicios.pInstrumentocdetalle = "CDP";
      }
      else if (lTran == "101" || lTran == "1101" || lTran == "3101") {
        this.inversionesServicios.pInstrumentocdetalle = "BONO";
      }
      else if (lTran == "102" || lTran == "1102" || lTran == "3102") {
        this.inversionesServicios.pInstrumentocdetalle = "CASHEQ";
      }
      else if (lTran == "103" || lTran == "1103" || lTran == "3103") {
  ...
  selectRegistro(registro: any) {
    ...
    if (!this.validarGrabar(false)) return;
  ...
  public crearDtoConsulta(): Consulta {
    ...
    consulta.addSubqueryPorSentencia('select isnull(proyeccioncapital,0) + isnull(proyeccioninteres,0) from tinvtablaamortizaciontmp a where a.cinvtablaamortizacion = t.cinvtablaamortizacion', 'total');
    consulta.addSubquery('tgencatalogodetalle', 'nombre', 'nestado', ' t.estadoccatalogo = i.ccatalogo and t.estadocdetalle = i.cdetalle');
    consulta.addSubqueryPorSentencia('select isnull(a.proyecciontasa,0) * 100 from tinvtablaamortizaciontmp a where a.cinvtablaamortizacion = t.cinvtablaamortizacion', 'nproyecciontasa');
  ...
  validarGrabar(iblnValidaCapital: boolean = true, iblnValidaValorDiferente: boolean = false): boolean
  {
    let lMensaje: string = "";
    ...
    else if (!this.estaVacio(this.mcampos.tasanueva) && this.inversionesServicios.pTasa == this.mcampos.tasanueva) {
      lMensaje = "LA NUEVA TASA DEBE SER DIFERENTE A LA TASA ACTUAL";
    }

    let lcapitalvalornominal: number = 0;
    let lcapitaltotal: number = 0;

    if (!this.estaVacio(this.instrumentoComponent.pInstrumento[0].valornominal) && this.instrumentoComponent.pInstrumento[0].valornominal != 0) lcapitalvalornominal = this.redondear(Number(this.instrumentoComponent.pInstrumento[0].valornominal),2);

    if (!this.estaVacio(this.rqMantenimiento.lregistrosTotales[0].capital) && this.rqMantenimiento.lregistrosTotales[0].capital != 0) lcapitaltotal = this.redondear(Number(this.rqMantenimiento.lregistrosTotales[0].capital),2);

    if (iblnValidaCapital && lcapitalvalornominal != lcapitaltotal)
    {
      lMensaje = "EL VALOR NOMINAL DEBE SER IGUAL AL TOTAL DEL CAPITAL";
    }

    let linteresvalornominal: number = 0;
    let linterestotal: number = 0;

    if (!this.estaVacio(this.inversionesServicios.pInteres) && this.inversionesServicios.pInteres != 0) linteresvalornominal = this.redondear(Number(this.inversionesServicios.pInteres),2);

    if (!this.estaVacio(this.rqMantenimiento.lregistrosTotales[0].interes) && this.rqMantenimiento.lregistrosTotales[0].interes != 0) linterestotal = this.redondear(Number(this.rqMantenimiento.lregistrosTotales[0].interes),2);

    if (iblnValidaValorDiferente && linteresvalornominal == linterestotal && lcapitalvalornominal == lcapitaltotal)
    {
      lMensaje = "NO HA VARIADO EL VALOR DEL CAPITAL Y/O INTERÉS";
    }
  ...
  grabar(): void {
    ...
    if (!this.validarGrabar(false, true)) return;
    ...
    this.rqMantenimiento.interesnuevo = this.rqMantenimiento.lregistrosTotales[0].interes;
    this.rqMantenimiento.capitalnuevo = this.rqMantenimiento.lregistrosTotales[0].capital;
      ...
      for (const i in this.lregistros) {
        if (this.lregistros.hasOwnProperty(i)) {
          ...
          if (this.lregistros[i].estadocdetalle == 'PEN') {
            ...
            this.lregistros[i].mdatos.nproyecciontasa = this.mcampos.tasanueva;
    ...
    for (const i in this.lregistros) {
      if (this.lregistros.hasOwnProperty(i)) {
        ...
        if (!this.estaVacio(this.lregistros[i].proyeccioncapital))
          this.rqMantenimiento.lregistrosTotales[0].capital = this.rqMantenimiento.lregistrosTotales[0].capital + Number(this.lregistros[i].proyeccioncapital);

        if (!this.estaVacio(this.lregistros[i].proyeccioninteres))
          this.rqMantenimiento.lregistrosTotales[0].interes = this.rqMantenimiento.lregistrosTotales[0].interes + Number(this.lregistros[i].proyeccioninteres);
      }
    }

    this.rqMantenimiento.lregistrosTotales[0].capital = this.redondear(this.rqMantenimiento.lregistrosTotales[0].capital, 2);
    this.rqMantenimiento.lregistrosTotales[0].interes = this.redondear(this.rqMantenimiento.lregistrosTotales[0].interes, 2);
