...
@Component({
  selector: 'app-pagosrf',
  templateUrl: 'pagosrf.html',
  providers: [InversionesServicios]
})
export class PagosrfComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  constructor(router: Router, dtoServicios: DtoServicios, private inversionesServicios: InversionesServicios, private confirmationService: ConfirmationService) {
    super(router, dtoServicios, 'tinvcontabilizacion', 'PAGOSRENTAFIJA', false, false);
  ...
  grabar(): void {
    ...
    if (this.rqMantenimiento.lregistrosTmp == undefined || this.rqMantenimiento.lregistrosTmp.length == 0) {
      ...
      if (this.estaVacio(this.inversionesServicios.pTotalapagar) || this.inversionesServicios.pTotalapagar == 0) {
        this.mostrarMensajeError("INGRESE EL VALOR CANCELADO");
        return;
      }
      else if (this.inversionesServicios.pTotalapagar <= 0) {
        this.mostrarMensajeError("VALOR CANCELADO DEBE SER MAYOR A CERO");
        return;
      }

      //else if (Math.abs(this.inversionesServicios.pDiferencia) > 1000) {
       // this.mostrarMensajeError("LA DIFERENCIA ENTRE EL VALOR CANCELADO Y EL CAPITAL + EL INTERÉS DEL DIVIDENDO NO DEBE SER MAYOR A +/- 5");
       // return;
     // }
      ...
      this.confirmationService.confirm({
        message: 'Está seguro de registrar el pago para su aprobación?',
        header: 'Confirmación',
  ...
  private grabaDetalle() {
    ...
    let lDiferencia: number = this.redondear(this.inversionesServicios.pTotalapagar - this.inversionesServicios.pSubtotal, 2);

    if (lDiferencia != 0) {
      this.inversionesServicios.ptablaamortizaregistro[0].valor = this.inversionesServicios.ptablaamortizaregistro[0].valor + lDiferencia;

      if (lDiferencia > 0) {
        this.inversionesServicios.ptablaamortizaregistro.push({ rubrocdetalle: 'ING', valor: lDiferencia, debito: false });
      }
      else {
        this.inversionesServicios.ptablaamortizaregistro.push({ rubrocdetalle: 'PERDID', valor: Math.abs(lDiferencia), debito: true });
      }
    }
  ...
  cancelarPago() {
    ...
    //this.cuotasComponent.lregistros[lIndiceExtracto].vintes = this.inversionesServicios.vintes;
    //this.cuotasComponent.lregistros[lIndiceExtracto].vcapital = this.inversionesServicios.vcapital

    this.cuotasComponent.lregistros[lIndiceExtracto].tabamo_valorcancelado = this.inversionesServicios.pTotalapagar;

    this.cuotasComponent.lregistros[lIndiceExtracto].comentariosdevolucion = this.inversionesServicios.pComentariosDevolucion;
  ...
  llenarEstado() {
    ...
    this.lestado.push({ label: 'PENDIENTES', value: 'PEN' });
