...
export class CuotasComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  public crearDtoConsulta() {
    ...
    rqConsulta.whereAdicional = rqConsulta.whereAdicional + " AND tinvtabamo.estadocdetalle <> 'PAG' and tinv.cinversion not in (select distinct cinversion from tinvtablaamortizacion where estadocdetalle = 'ENVAPR') ";

    let lwhereEstado: string = "";

    if (!this.estaVacio(this.pEstadoCondicion) && this.pEstadoCondicion != "..." && this.pEstadoCondicion != "TODOS") {
      lwhereEstado = " AND tinvtabamo.estadocdetalle = '" + this.pEstadoCondicion + "' ";
    }
  ...
  edit(row: any, index: number) {
    ...
    //this.inversionesServicios.pEmisorcdetalle = row.emisorcdetalle;
    ...
    this.inversionesServicios.pComentariosDevolucion = row.comentariosdevolucion;
    //this.inversionesServicios.vintes=row.vintes;
    //this.inversionesServicios.vcapital= row.vcapital;
    ...
    let lDividendo: number = 0;

    if (!this.estaVacio(row.tabamo_capital) && Number(row.tabamo_capital) != 0) lDividendo = this.redondear(Number(row.tabamo_capital),2);

    if (!this.estaVacio(row.tabamo_interes) && Number(row.tabamo_interes) != 0) lDividendo = lDividendo + this.redondear(Number(row.tabamo_interes),2);

    let linteresimplicito: number = 0;

    try
    {
      if (!this.estaVacio(row.diferenciainteresimplicito)) 
        if (Number(row.diferenciainteresimplicito) != 0) 
          linteresimplicito = Number(row.diferenciainteresimplicito);
    }
    catch (ex)
    {
    ...
    linteresimplicito=0;
    //if (row.instrumentocodigo == 'OBLIGA' || row.instrumentocodigo == 'TITULA' || row.instrumentocodigo == 'VALTIT')
    if (!this.estaVacio(linteresimplicito) && linteresimplicito != 0)
    {
      this.inversionesServicios.ptablaamortizaregistro.push({ rubrocdetalle: 'BANCOS', valor: lDividendo, debito: true });

      let lCapital:number = 0;

      if (!this.estaVacio(row.tabamo_capital) && Number(row.tabamo_capital) != 0) lCapital = this.redondear(Number(row.tabamo_capital),2);

      //      linteresimplicito = linteresimplicito + (row.diferenciainteresimplicito - (linteresimplicito - lDividendo + lCapital));
      linteresimplicito = linteresimplicito + ( - (linteresimplicito - lDividendo + lCapital));

      this.inversionesServicios.ptablaamortizaregistro.push({ rubrocdetalle: 'INT', valor: this.redondear(linteresimplicito,2), debito: false });


      if (Number(row.diferenciainteresimplicito >= 0))
      {

        this.inversionesServicios.ptablaamortizaregistro.push({ rubrocdetalle: 'CAP', valor: this.redondear(lDividendo - linteresimplicito,2), debito: false });

        //this.inversionesServicios.ptablaamortizaregistro.push({ rubrocdetalle: 'ING', valor: this.redondear(row.diferenciainteresimplicito,2), debito: true });

        this.inversionesServicios.ptablaamortizaregistro.push({ rubrocdetalle: 'CAP', valor: this.redondear(row.diferenciainteresimplicito,2), debito: false });
  
      }
      else
      {

        this.inversionesServicios.ptablaamortizaregistro.push({ rubrocdetalle: 'CAP', valor: lCapital, debito: false });

      //  this.inversionesServicios.ptablaamortizaregistro.push({ rubrocdetalle: 'ING', valor: Math.abs(this.redondear(row.diferenciainteresimplicito,2)), debito: false });
    ...
    else
    {
      ...
      this.inversionesServicios.ptablaamortizaregistro.push({ rubrocdetalle: 'BANCOS', valor: this.redondear(lDividendo,2), debito: true });
      this.inversionesServicios.ptablaamortizaregistro.push({ rubrocdetalle: 'CAP', valor: this.redondear( row.tabamo_capital,2), debito: false });
      this.inversionesServicios.ptablaamortizaregistro.push({ rubrocdetalle: 'INT', valor: this.redondear(row.tabamo_interes,2), debito: false });

    }

    if (!this.estaVacio(row.tabamo_capital) && this.redondear(Number(row.tabamo_capital),2) != 0) 
    {

      this.inversionesServicios.ptablaamortizaregistro.push({ rubrocdetalle: 'CTORDE', valor: this.redondear(row.tabamo_capital,2), debito: false });
      this.inversionesServicios.ptablaamortizaregistro.push({ rubrocdetalle: 'CTORAC', valor: this.redondear(row.tabamo_capital,2), debito: true });

    }
    
    //lDividendo = this.redondear(Number(row.tabamo_capital),2);
