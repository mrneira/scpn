...
@Component({
  selector: 'app-contabilidadentidad',
  templateUrl: 'contabilidadentidad.html'
})
export class ContabilidadentidadComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  constructor(router: Router, dtoServicios: DtoServicios) {
    super(router, dtoServicios, 'tinvplantillacontable', 'CONTABILIDADENTIDAD', false, false);
  ...
  validarCabecera(): string {
    ...
    else if (this.estaVacio(this.mfiltros.instrumentocdetalle)) {
  ...
  actualizar() {
    ...
    {
      this.registro.entidadccatalogo = 1213;
    }
    else
    {
  ...
  public crearDtoConsulta(): Consulta {
    ...
    this.mfiltrosesp.rubrocdetalle = " in ('CAP','INT','ING','GASTO','CTORDE','CTORAC','COMISI','IMPUES','PERDID') ";
    //this.mfiltrosesp.rubrocdetalle = " in ('GASTO') ";
  ...
  llenarConsultaCatalogos(): void {
    ...
    const mfiltrosparam = { 'ccatalogo': 'in (1213)' };
    ...
    consInstrumento.cantidad = 500;
    this.addConsultaPorAlias('INSTRUMENTO', consInstrumento);

    const mfiltrosoperacion = { 'ccatalogo': 1220 };
    const mfiltrosopeesp = { "cdetalle": " in ('COMPRA', 'RECUP', 'ING', 'VARPRE') "};
    const consOperacion = new Consulta('tgencatalogodetalle', 'Y', 't.nombre', mfiltrosoperacion, mfiltrosopeesp);
    consOperacion.cantidad = 500;
    this.addConsultaPorAlias('OPERACION', consOperacion);

    const mfiltrosEspRubro = { "cdetalle": " in ('CAP', 'INT','ING','GASTO','CTORDE','CTORAC','COMISI','IMPUES','PERDID') "};

    const mfiltrosrubro = { 'ccatalogo': 1219 };
    const consRubro = new Consulta('tgencatalogodetalle', 'Y', 't.nombre', mfiltrosrubro, mfiltrosEspRubro);
