...
@Component({
  selector: 'app-ingresosuministros',
  templateUrl: 'ingresosuministros.html'
})
export class IngresoSuministrosComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  public llenaListaTipoIngreso(pLista: any, pListaResp, campopk = 'pk', agregaRegistroVacio = true, componentehijo = null): any {    
    for(let i in pListaResp){
      ...
      if(reg.cdetalle !== 'DEVFUN' && reg.cdetalle !== 'DEVSUM'){
  ...
  calcularTotales() {
    ...
    this.cabeceraComponent.registro.subtotaliva = sumatoriobaseiva;
    this.cabeceraComponent.registro.iva = (this.cabeceraComponent.registro.subtotaliva * 0.12);
