...
export class CodificarProductoComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  public codificado: boolean = false;
  public procesando:boolean= false;

  public ltipoingresocdetalle: SelectItem[] = [{ label: '...', value: null }];

  public ltipomarca: SelectItem[] = [{ label: '...', value: null }];
  ...
  constructor(router: Router, dtoServicios: DtoServicios) {
    super(router, dtoServicios, 'ABSTRACT', 'CODIFICARPRODUCTO', false);
  }
  ...
  grabar(): void {
    ...
    this.rqMantenimiento.mdatos.memocodificacion = this.cabeceraComponent.registro.memocodificacion;
    this.rqMantenimiento.mdatos.codificado = this.codificado;
  ...
  fijarLovIngresosSelec(reg: any): void {
    if (reg.registro !== undefined) {
      ...
      this.codificado = reg.registro.codificado;
  ...
  consultarCatalogos(): void {
    ...
    conTipoIngreso.cantidad = 10000;
    this.addConsultaCatalogos('TIPOINGRESO', conTipoIngreso, this.ltipoingresocdetalle, super.llenaListaCatalogo, 'cdetalle');

    this.catalogoDetalle = new CatalogoDetalleComponent(this.router, this.dtoServicios);
    this.catalogoDetalle.mfiltros.ccatalogo = 1302;
    const conMarca = this.catalogoDetalle.crearDtoConsulta();
    conMarca.cantidad = 1000;
    this.addConsultaCatalogos('TMARCA', conMarca, this.ltipomarca, super.llenaListaCatalogo, 'cdetalle');

    this.catalogoDetalle = new CatalogoDetalleComponent(this.router, this.dtoServicios);
    this.catalogoDetalle.mfiltros.ccatalogo = 1304;
    const conEstadoIngreso = this.catalogoDetalle.crearDtoConsulta();
    conEstadoIngreso.cantidad = 10000;
  ...
  validarIngreso(): string {
    ...
    if (this.cabeceraComponent.registro.memocodificacion === null || this.cabeceraComponent.registro.memocodificacion === undefined) {
      return 'INGRESE MEMO DE CODIFICACIÓN ';
    }

    /*for (const i in this.detalleComponent.lregistros) {
      const reg = this.detalleComponent.lregistros[i];
      contadorRegistros++;
      if (reg.serial === '' || reg.serial === undefined) {
