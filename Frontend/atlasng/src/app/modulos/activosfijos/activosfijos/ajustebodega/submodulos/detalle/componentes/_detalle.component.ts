...
export class DetalleComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  productoBlur(reg: any, index: number){
    ...
    const consulta = new Consulta('tacfproducto', 'N', '', {'codigo' : reg.mdatos.codigo,'movimiento': true, 'ctipoproducto':3}, {});
  ...
  private cerrarDialogo(): void {
    this.registro.mdatos.diferencia =  this.registro.fisico-this.registro.mdatos.stock ;
   
  }

  cambiarCantidad(indice,event): void {
    this.lregistros[indice].mdatos.diferencia =  this.lregistros[indice].fisico-this.lregistros[indice].mdatos.stock;
