...
@Component({
  selector: 'app-tth-anticiposbuzonth',
  templateUrl: 'anticiposbuzonth.html'
})
export class AnticiposbuzonthComponent extends BaseComponent implements OnInit, AfterViewInit {
 ...
 // private lEstados: SelectItem[] = [{ label: '...', value: null },{ label: 'INGRESADA', value: 'ING' },{ label: 'APROBADA', value: 'APR' },{ label: 'NEGADA', value: 'NEG' }];
  ...
  consultarSolicitud() {
    ...
    this.rqConsulta.CODIGOCONSULTA = 'ANTICIPOS_LISTA_SOLICITUD';
    this.rqConsulta.storeprocedure = "sp_AntListaSolicitudes";
    ...
    this.dtoServicios.ejecutarConsultaRest(this.getRequestConsulta('C'))
      .subscribe(
      resp => {    
        this.lSolicitudAnticipo = resp.ANTICIPOS_LISTA_SOLICITUD;
        this.filtrarListaEstado();
...
console.log("lCapacidadEndeudamiento",resp);
  ...
  verDialogoSolicitud(reg){
    ...
    this.antestado = reg.antestado == "ING" ? "INGRESADA" : reg.antestado == "APR" ? "APROBADA" : "NEGADA";
  ...
  guardarDialogoSolicitud(){
     this.rqConsulta = [];
    this.mostrarDialogoSolicitud = false;
    let respuesta = this.selectedvalue === 'APR' ? 'APROBADA' : 'NEGADA';
    const mradicacion = JSON.parse(sessionStorage.getItem('mradicacion'));
    this.rqConsulta.CODIGOCONSULTA = 'ANTICIPOS_CAMBIA_ESTADO';
    this.rqConsulta.storeprocedure = "sp_AntCambiarEstado";
    ...
    this.dtoServicios.ejecutarConsultaRest(this.getRequestConsulta('C'))
            ...
            resp => {                     
               if (resp.cod === 'OK') {    
                  if(resp.ANTICIPOS_CAMBIA_ESTADO[0].respuesta == "ERROR")
                    this.dtoServicios.manejoError("LA SOLICITUD N°. " + this.cantsolicitud + " NO HA PODIDO SER PROCESADA CORRECTAMENTE");
                  else {
                    if(resp.ANTICIPOS_CAMBIA_ESTADO[0].comprobanteIngresado != "0"){
                        this.generaDocumentoAnticipo_3(resp.ANTICIPOS_CAMBIA_ESTADO[0].comprobanteIngresado, "pdf");  
                    }
                    super.mostrarMensajeSuccess(resp.ANTICIPOS_CAMBIA_ESTADO[0].respuesta); 
