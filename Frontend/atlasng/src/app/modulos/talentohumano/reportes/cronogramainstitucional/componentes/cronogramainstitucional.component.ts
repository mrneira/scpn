...
@Component({
  ...
  templateUrl: 'cronogramainstitucional.html'
})
...
export class CronogramaComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  public mostrarVacacion :boolean = false;
  ...
  onClikCalendar(reg: any) {
    ...
    if(this.events[id].mdatos.nevento=='VAC'){
    this.mostrarVacacion= true;
  }
  if(this.events[id].mdatos.nevento=='FER' || this.events[id].mdatos.nevento=='EVA' ){
    this.mostrarFeriado= true;
  }
  
  if(this.events[id].mdatos.nevento=='PER'){
  ...
  obtnerdatos() {
    this.rqConsulta.CODIGOCONSULTA = 'CRONOGRAMAINS';
