...
export class EgresoComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  constructor(router: Router, dtoServicios: DtoServicios) {
    super(router, dtoServicios, 'tnomjubilaciontotal', 'EGRESO', false, false);
  ...
  crearNuevo() {
    if (this.estaVacio(this.mcampos.cjubilacion)){
      this.mostrarMensajeError("ELIJA PRIMERO UNA JUBILACIÓN PARA MANTENIMIENTO");
      return;
    }
    super.crearNuevo();
  ...
  consultar() {
    if (this.estaVacio(this.mcampos.cjubilacion)) {
      this.mostrarMensajeError('CREE UNA NUEVA JUBILACION PARA REALIZAR EL MANTENIMIENTO');
      return;
    }
  ...
  public postQuery(resp: any) {
    ...
    let totale=0;
    if (!this.estaVacio(resp.EGRESO) && resp.EGRESO.length>0) {
