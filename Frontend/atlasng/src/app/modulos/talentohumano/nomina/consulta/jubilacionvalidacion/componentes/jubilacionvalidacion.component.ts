...
@Component({
  selector: 'app-jubilacionvalidacion',
  templateUrl: 'jubilacionvalidacion.html'
})
export class JubilacionvalidacionComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  constructor(router: Router, dtoServicios: DtoServicios, public appService: AppService) {
    super(router, dtoServicios, 'tnomjubilacion', 'LIQUIDACIONGENERAL', false);
  }
  ngOnInit() {
...
aprobarPago(reg:any){
  ...
  this.rqMantenimiento.mdatos.registro = reg;
  super.grabar();
}
  ...
  private fijarFiltrosConsulta() {
    this.mfiltros.cerrada= true;
    this.mfiltrosesp.ccomprobante = 'in (SELECT i.ccomprobante FROM tconcomprobante i '+
    'WHERE i.anulado = 0 AND i.aprobadopresupuesto = 1 AND i.actualizosaldo=0)';
  }
  IrAComprobanteContable(reg){
    ...
    this.router.navigate([opciones['path']], {skipLocationChange: true, 
                                              queryParams: {comprobante: JSON.stringify({ccomprobante: reg.ccomprobante,vienedelibromayor:false,
                                                                                        cmodulo: 10 })}});
    this.appService.titulopagina = opciones['tit'];
  }
