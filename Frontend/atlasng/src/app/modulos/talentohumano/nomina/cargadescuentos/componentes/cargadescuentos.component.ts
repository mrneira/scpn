...
@Component({
  selector: 'app-cargadescuentos',
  templateUrl: 'cargadescuentos.html'
})

export class CargadescuentosComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  constructor(router: Router, dtoServicios: DtoServicios) {
    super(router, dtoServicios, 'tnomdescuentopersona', 'DECUENTOPERSONA', false, false);
...
fijarLovParametroSelec(reg: any): void {
  ...
  onSelectArchivoFirma(event) {
    if(this.estaVacio(this.mcampos.mescdetalle)){
      super.mostrarMensajeError("SELECIONE UN MES");
      return;
    }
    if(this.estaVacio(this.mcampos.anio)){
      super.mostrarMensajeError("SELECIONE UN AÑO VÁLIDO");
      return;
    }
  ...
  obtnerdatos() {
    this.rqConsulta.CODIGOCONSULTA = 'CARGADESCUENTOSROL';
    ...
    this.dtoServicios.ejecutarConsultaRest(this.getRequestConsulta('C'))
      ...
      resp => {
        this.lregistros = [];
        if (resp.cod === 'OK') {
          this.lregistros = resp.CARGADESCUENTOS;
          this.lregistrosProb=resp.CARGADESCUENTOSERROR;
