...
@Component({
  selector: 'app-ajustesfindemes',
  templateUrl: 'ajustesfindemes.html'
})
export class AjustesfindemesComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.cargarFperiodoFinAnterior();
    //this.comprobanteComponent.registro.mdatos.fcontable = this.integerToDate(20181031);
  }
  ...
  cargarFperiodoFinAnterior() {

    this.rqConsulta.CODIGOCONSULTA = 'CON_FPERIODOFINANTERIOR';
    this.msgs = [];
  ...
  private manejaRespuesta(resp: any) {
    this.comprobanteComponent.registro.mdatos.fcontable = this.integerToDate(resp.CON_FPERIODOFINANTERIOR);
    this.FPERIODOFIN = this.integerToFormatoFecha(resp.CON_FPERIODOFINANTERIOR) ;
  }
}
