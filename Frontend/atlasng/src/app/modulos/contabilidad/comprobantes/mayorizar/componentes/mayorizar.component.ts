...
@Component({
  selector: 'app-mayorizar',
  templateUrl: 'mayorizar.html'
})
export class MayorizarComponent extends BaseComponent implements OnInit, AfterViewInit {
  [x: string]: any;
  ...
  public postQuery(resp: any) {
    ...
    this.cuadrado = this.comprobanteComponent.registro.cuadrado;
  ...
  mayorizar(): void {
    this.confirmationService.confirm({
      message: 'Está seguro de mayorizar el comprobante?',
      header: 'Mayorización de comprobantes',
      icon: 'fa fa-question-circle',
      accept: () => {
        this.rqMantenimiento.mayorizar = true;
        this.grabar();
      },
      reject: () => {
      }
  ...
  public postCommit(resp: any) {
    ...
    if (resp.mayorizado === 'OK') {
      this.comprobanteComponent.registro.actualizosaldo = true;
    }
    else{
      this.actualizosaldo = false;
      this.msgs.push({ severity: `error`, summary: resp.mayorizado });
  ...
  mostrarlovcomprobantes(): void {
    this.lovcomprobantes.mfiltros.cuadrado = true;
