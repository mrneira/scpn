...
export class DetalleComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  private manejaRespuesta(resp: any, index: number) {
    ...
    if (resp.CONSULTACUENTACONTABLE.length === 1) {
      ...
      this.lregistros[index].mdatos.ncuenta = cuentacontable[0].nombre;
      this.lregistros[index].cmoneda = cuentacontable[0].cmoneda;
      this.lregistros[index].mdatos.tipoplancdetalle = cuentacontable[0].tipoplancdetalle;
    } else {
      this.lregistros[index].ccuenta = undefined;
      this.lregistros[index].mdatos.ncuenta = undefined;
      this.lregistros[index].cmoneda = undefined;
      this.lregistros[index].mdatos.tipoplancdetalle =undefined;
      this.mostrarMensajeError("CUENTA CONTABLE NO EXISTE O INACTIVA O NO ES DE MOVIMIENTO");
    }
    this.rqConsulta.CODIGOCONSULTA = null;
  }
