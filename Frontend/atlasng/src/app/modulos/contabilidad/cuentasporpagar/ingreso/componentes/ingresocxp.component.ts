...
@Component({
  selector: 'app-ingresocxp',
  templateUrl: 'ingresocxp.html'
})
export class IngresocxpComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  mostrarlovingresos(): void {
    if (this.comprobanteComponent.registro.cplantilla === undefined || this.comprobanteComponent.registro.cpersona === undefined) {
      super.mostrarMensajeError("SELECCIONE PLANTILLA Y PROVEEDOR PARA CONTINUAR");
      return;
    }
  ...
  calcularTotalFactura(){
    this.mcampos.totalfactura = this.redondear(this.comprobanteComponent.registro.baseimponible,2) +
                                this.redondear(this.comprobanteComponent.registro.montoiva,2) +
                                this.redondear(this.comprobanteComponent.registro.montoice, 2);
  }
  calcularIVA() {
    const porcentaje = this.lporcentajeivacdetalle.find(x => x.value === this.comprobanteComponent.registro.porcentajeivacdetalle);
    if (porcentaje !== undefined) {
      this.comprobanteComponent.registro.montoiva = this.redondear((Number(porcentaje.label) / 100) * this.comprobanteComponent.registro.baseimpgrav,2);
    }
    this.calcularTotalFactura();
  }
  ...
  calcularICE() {
    ...
    if (porcentaje !== undefined) {
      this.comprobanteComponent.registro.montoice = this.redondear((Number(porcentaje.label) / 100) * this.comprobanteComponent.registro.baseimpgrav,2);
    }
    this.calcularTotalFactura();
  }
  ...
  calcularRetIVABienes() {
    ...
    this.comprobanteComponent.registro.valorretbienes = this.redondear((Number(porcentaje.label === "..." ? 0 : porcentaje.label) / 100) * this.comprobanteComponent.registro.montoivabienes, 2);
    this.calcularTotalRetencionIVA();
  }
  ...
  calcularRetIVAServicios() {
    ...
    this.comprobanteComponent.registro.valorretservicios = this.redondear((Number(porcentaje.label === "..." ? 0 : porcentaje.label) / 100) * this.comprobanteComponent.registro.montoivaservicios,2);
    this.calcularTotalRetencionIVA();
  }
  ...
  calcularTotalRetencionIVA() {
    ...
    this.totalretencionesiva = this.redondear(this.comprobanteComponent.registro.valorretservicios,2) + this.redondear(this.comprobanteComponent.registro.valorretbienes,2);
    this.calcularTotalRetenciones();
  }
  ...
  calcularTotalRetenciones() {
    ...
    this.totalretenciones = this.redondear(this.totalretencionesiva,2) + this.redondear(this.detalleComponent.totalRetencionesIR,2);
    const totalretenciones = this.totalretenciones;
    this.comprobanteComponent.registro.valorpagar = ( this.redondear(baseimponible,2) + this.redondear(montoiva,2) + 
    this.redondear(montoice,2) - this.redondear(totalretenciones,2) - this.redondear(this.comprobanteComponent.registro.valormulta,2)
    - this.redondear(this.comprobanteComponent.registro.valornotascredito,2));
  }
  ...
  descargarReporteComprobanteContable(): void {
    this.jasper.nombreArchivo = "Diario general por registro de facturas diarias de parqueadero";
    // Agregar parametros
  ...
  validarDatosCuentaPorPagar(): string {
    ...
    if (this.comprobanteComponent.registro.ccuentaafectacion === null || this.comprobanteComponent.registro.ccuentaafectacion === undefined) {
      mensaje += 'Seleccione cuenta contable de afectación <br />';
    }

    if (this.comprobanteComponent.registro.centrocostoscdetalle === null || this.comprobanteComponent.registro.centrocostoscdetalle === undefined) {
      mensaje += 'Seleccione centro de costos <br />';
    }
    ...
    if (this.estaVacio(this.comprobanteComponent.registro.formapagocdetalle)) {
      mensaje += 'Ingrese la forma de pago <br />';
    }
    if (this.estaVacio(this.comprobanteComponent.registro.comentario) || this.comprobanteComponent.registro.comentario.length==0) {
      mensaje += 'Ingrese el comentario <br />';
    }
    if (!this.comprobanteComponent.registro.exentoretencion) {
      ...
      //CCA cambios para no gravar sobre la base imponible
  ...
  validarNumeroDocumentoSustento():void{
    ...
    const conNumeroDocumentoSustento = new Consulta('tconcuentaporpagar', 'N', '', { numdocumentosustento: this.comprobanteComponent.registro.numdocumentosustento, 
                                      cpersona: this.comprobanteComponent.registro.cpersona},
                                      { estadocxpcdetalle: ' IN (\'INGRES\',\'CONTAB\',\'PAGADO\')'}, {});
