...
@Component({
  ...
  templateUrl: 'ingresoGar.html'
})

export class IngresoGarComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  avaluoComponet: AvaluoComponent;
  ...
  public crearDtoConsulta() {
    ...
    const conAval = this.avaluoComponet.crearDtoConsulta();
    this.addConsultaPorAlias(this.avaluoComponet.alias, conAval);
  ...
  private fijarFiltrosConsulta() {
    ...
    this.avaluoComponet.mfiltros.coperacion = this.coperacion;
    this.avaluoComponet.fijarFiltrosConsulta();
  }

  validaFiltrosConsulta(): boolean {
    return this.garComponent.validaFiltrosRequeridos() &&
      this.camposComponent.validaFiltrosRequeridos() &&
      this.avaluoComponet.validaFiltrosRequeridos();
  }
  ...
  public postQuery(resp: any) {
    ...
    this.avaluoComponet.postQuery(resp);
  ...
  grabar(): void {
    ...
    super.addMantenimientoPorAlias(this.avaluoComponet.alias, this.avaluoComponet.getMantenimiento(3));
    super.grabar();
  }
  ...
  validaGrabar() {
    return this.garComponent.validaGrabar() && this.camposComponent.validaGrabar() && this.avaluoComponet.validaGrabar(this.mcampos.coperacion);
  }
  ...
  public postCommit(resp: any) {
    ...
    this.avaluoComponet.postCommit(resp, this.getDtoMantenimiento(this.avaluoComponet.alias));

    if (resp.cod === 'OK') {
      this.mcampos.coperacion = this.garComponent.registro.coperacion;
      this.avaluoComponet.mcampos.coperacion = this.garComponent.registro.coperacion;
  ...
  fijarLovPersonaSelec(reg: any): void {
    if (reg.registro !== undefined) {
      ...
      this.avaluoComponet.mcampos.cpersona = reg.registro.cpersona;
      this.coperacion = 0;
  ...
  fijarLovOperacionSelec(reg: any): void {
    ...
    if (!this.estaVacio(this.mcampos.coperacion)) {
      this.avaluoComponet.mcampos.coperacion = this.mcampos.coperacion;
      this.visible = false;
    }
    this.consultar();
