...
export class SolicitudComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  private lvalidacionprestamo: any = [];

  public aplicaPrestamo: boolean = true; // RRO 20210315
  ...
  public mostrar(event: any): any {
    this.aplicaPrestamo = true;//RRO 20210315
  ...
  public consultarDatosPrestamo() {
    ...
    tipoProductoComponent.rqConsulta.mdatos.validacionplazofijo = undefined;
    if (this.registro.cproducto == 1 && this.registro.ctipoproducto == 21) {
      tipoProductoComponent.rqConsulta.mdatos.validacionplazofijo = true;
    }
  ...
  private manejaRespuestaDatosProductoCartera(resp: any) {
    ...
    if (resp.cod === 'OK' && !this.estaVacio(resp.TIPOPRODUCTO)) {
      ...
      this.registro.mdatos.requieregarante = tp.mdatos.requieregarante;
      this.registro.mdatos.consolidado = tp.mdatos.consolidado;
      this.registro.mdatos.csegmento = tp.mdatos.csegmento;
      this.registro.mdatos.gracia = tp.mdatos.gracia;
      this.registro.mdatos.absorberoperaciones = tp.mdatos.absorberoperaciones;
      this.registro.montooriginal = this.estaVacio(tp.mdatos.montodefault) ? undefined : tp.mdatos.montodefault;
      this.registro.montooriginal = this.estaVacio(resp.montovalidado) ? this.registro.montooriginal : resp.montovalidado;
      this.lrangos = tp.mdatos.RANGOS;
      if (this.registro.montooriginal > 0) {
        this.cargarCuotas(this.registro.montooriginal);
        this.cargarFlujo(tp.mdatos.cflujo);
      }
    }
    if (resp.cod === 'OK' && !this.estaVacio(resp.MONTONOVALIDO)) {
      this.mostrarMensajeError(resp.MONTONOVALIDO);
      this.registro.numerocuotas=0;
      this.aplicaPrestamo = false; //RRO 20210315
    }
  }

  recargarSimulacion() {
    if(this.aplicaPrestamo){ // RRO 20210315
     this.eventoSimulacion.emit(); 
     if (this.registro.mdatos.gracia) {
      this.registro.cuotasgracia = this.registro.numerocuotas - 1;
      }
      else {
      this.registro.cuotasgracia = null
      }
