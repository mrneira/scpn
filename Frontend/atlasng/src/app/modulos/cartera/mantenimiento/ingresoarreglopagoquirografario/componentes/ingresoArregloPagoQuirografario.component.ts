...
import { OperacionRubrosArregloPagoQuirografarioComponent } from './_operacionRubrosArregloPagoQuirografario.component';
import { RubrosArregloPagoQuirografarioComponent } from './_rubrosArregloPagoQuirografario.component';
import { LovOperacionCarteraComponent } from '../../../lov/operacion/componentes/lov.operacionCartera.component';
import { SelectItem } from 'primeng/primeng';

@Component({
  selector: 'app-ing-arreglo-pagos-Quirografario',
  templateUrl: 'ingresoArregloPagoQuirografario.html'
})
export class IngresoArregloPagoQuirografarioComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  @ViewChild(CondicionesArregloPagoQuirografarioComponent)
  condicionesArreglo: CondicionesArregloPagoQuirografarioComponent;

  @ViewChild(OperacionRubrosArregloPagoQuirografarioComponent)
  operacionRubrosArreglo: OperacionRubrosArregloPagoQuirografarioComponent;

  @ViewChild(RubrosArregloPagoQuirografarioComponent)
  rubrosArreglo: RubrosArregloPagoQuirografarioComponent;
  ...
  public mostrarDialogoConfirmacion = false
  ...
  grabar(): void {
    ...
    if (this.estaVacio(this.condicionesArreglo.registro.valorcuota) && this.estaVacio(this.condicionesArreglo.registro.numerocuotas)) {
      super.mostrarMensajeError('NÚMERO DE CUOTAS O VALOR DE CUOTA REQUERIDOS');
      return;
    }
    if (parseInt(this.condicionesArreglo.registro.numerocuotas) < 1) {
      super.mostrarMensajeError('EL NÚMERO DE CUOTAS O VALOR DE CUOTA DEBE SER MAYOR A CERO');
      return;
    }
    if (this.estaVacio(this.condicionesArreglo.mcampos.destino)) {
      super.mostrarMensajeError('DESTINO DEL CRÉDITO ES REQUERIDO');
      return;
    }
    if (this.estaVacio(this.condicionesArreglo.mcampos.numerocuotasprorrateos)) {
      super.mostrarMensajeError('ES OBLIGATORIO INGRESAR EL NÚMERO DE CUOTA DESDE DONDE DESEA PRRORRATEAR LOS RUBROS VENCIDOS');
      return;
    }
    if (parseInt(this.condicionesArreglo.mcampos.numerocuotasprorrateos) > parseInt(this.condicionesArreglo.registro.numerocuotas)) {
    ...
    if (parseInt(this.condicionesArreglo.mcampos.numerocuotasprorrateos) < 1) {
    ...
    if (this.estaVacio(this.condicionesArreglo.ltabla)) {
      this.mostrarMensajeError('LA SIMULACIÓN DE LA TABLA DE AMORTIZACIÓN ES REQUERIDA');
      return;
    }
    if (this.requieregarante) {
      if (!this.capacidadGarante.aprobado) {
        this.mostrarMensajeError('LA NEGOCIACIÓN REQUIERE LA APROBACIÓN DE UN GARANTE');
        return;
      }
    }
  ...
  confirmarGrabar() {
    ...
    this.condicionesArreglo.selectRegistro(this.condicionesArreglo.configGeneraTabla["registro"]);
    ...
    if (this.capacidadDeudor.habilitaAbsorcion) {
      ...
      this.rqMantenimiento['prorrateo'] = this.condicionesArreglo.configGeneraTabla["mcampos"]["numerocuotasprorrateos"];
  ...
  grabarSimulacion(): void {
    ...
    this.condicionesArreglo.mcampos.cmodulonegociacion = this.cmodulo;
    this.condicionesArreglo.mcampos.cproductonegociacion = this.cproducto;
    this.condicionesArreglo.mcampos.tipoarreglonegociacion = this.tipoarreglo;
    this.condicionesArreglo.mcampos.ctipoproductonegociacion = this.ctipoproducto;
  ...
  public cargarPagina(reg: any) {
    ...
    const titpagina = sessionStorage.getItem('m') + '-' + tranpagina + ' MANTENIMIENTO DE SOLICITUD';
    opciones['path'] = sessionStorage.getItem('m') + '-' + tran;
    opciones['tit'] = sessionStorage.getItem('m') + '-' + tran + ' MANTENIMIENTO DE SOLICITUD';
  ...
  fijarLovPersonaSelec(reg: any): void {
    if (this.lovOperacion.lregistros.length > 0) {
      this.mostrarMensajeError('PARA PROCEDER A SELECCIONAR OTRO USUARIO, POR FAVOR HAGA CLIC EN LIMPIAR Y CONTUNE CON EL PROCESO');
      return;
    }
    this.estatusBaja = false;
    if (reg.registro !== undefined) {
      if(
        reg["registro"]["mdatos"]
        && reg["registro"]["mdatos"]["fnacimiento"]
      ){
        const fnacimiento = parseInt((String(reg["registro"]["mdatos"]["fnacimiento"]).split("-")[0]) + String(reg["registro"]["mdatos"]["fnacimiento"]).split("-")[1] + String(reg["registro"]["mdatos"]["fnacimiento"]).split("-")[2])
        const factual = parseInt(String(this.dtoServicios["mradicacion"]["ftrabajo"]));
        const edad = factual - fnacimiento;
        this.mcampos.anioedadactual = parseInt(String(edad).substring(0, 2));
        /*if(parseInt(String(edad).substring(0, 2)) >= 60){
          super.mostrarMensajeError("NO ESTA PERMITIDO LA NEGOCIACIÓN DE PAGOS, PARA LOS SOCIOS QUE TIENE 60 AÑOS DE EDAD O MÁS.");
          return;
        }*/
        if (
          reg['registro']['mdatos'] &&
          reg['registro']['mdatos'] instanceof Object &&
          (reg['registro']['mdatos']['tipoestado'] === "BAJ" || reg['registro']['mdatos']['tipoestado'] === "BLQ")
        ) {
          this.estatusBaja = true;
        }
        this.capacidadDeudor.ingresosCapacidadComponent.setStatusBaja(this.estatusBaja);
        ...
        super.mostrarMensajeError("EL SOCIO NO TIENE DEFINIDIO UNA FECHA DE NACIMIENTO");
        return;
      }
    }
  }
  ...
  fijarLovOperacionSelec(reg: any): void {
    if (
      reg &&
      reg instanceof Object &&
      reg['registro'] &&
      reg['registro'] instanceof Object &&
      reg['registro']['mdatos'] &&
      reg['registro']['mdatos'] instanceof Object &&
      reg['registro']['mdatos']['nproducto'] &&
      //reg['registro']['mdatos']['nproducto'] === "QUIROGRAFARIO" &&
      String(reg['registro']['mdatos']['requieregarante']) != "undefined"
    ) {
      this.requieregarante = (reg['registro']['mdatos']['requieregarante'] === null) ? false : reg['registro']['mdatos']['requieregarante'];
      ...
      this.condicionesArreglo.registro.mdatos.numerocuotas = reg.registro.numerocuotas;
      this.condicionesArreglo.registro.mdatos.ctipoarreglopago = reg['registro']['mdatos']['nproducto'];
      this.condicionesArreglo.registro.mdatos.ntipoproducto = reg.registro.mdatos.ntipoproducto;
      ...
      super.mostrarMensajeError("LA TRANSACCIÓN 52 - MÓDULO 7; SOLO SE ENCUENTRA HABILITADA PARA EL PRODUCTO QUIROGRAFARIO");
    }
  }
  ...
  consultarRubrosArregloParam(): any {
    ...
    /*if(this.tipoarreglo == "RES" && this.mcampos.anioedadactual >= 60){
      super.mostrarMensajeError("NO ESTA PERMITIDO LA NEGOCIACIÓN DE PAGOS RESTRUCTURADA, PARA LOS SOCIOS QUE TIENE 60 AÑOS DE EDAD O MÁS.");
      return;
    }*/
    this.habilitamensaje = false;
    this.condiciones = false;
    this.condicionesArreglo.registro.ctipoarreglopago = this.tipoarreglo;
    this.capacidadDeudor.registro.ctipoarreglopago = this.tipoarreglo;
    this.encerarConsultaCatalogos();
