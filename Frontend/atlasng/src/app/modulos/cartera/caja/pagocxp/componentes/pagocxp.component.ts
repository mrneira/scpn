...
@Component({
  selector: 'app-pago-cxp',
  templateUrl: 'pagocxp.html'
})
export class PagocxpComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  fijarLovOperacionSelec(reg: any): void {
    ...
    this.consultarCxp(this.mcampos.coperacion);
  }
  ...
  consultarCxp(coperacion: string): any {
    this.encerarConsultaCatalogos();

    const mfiltroscxp: any = { 'coperacion': coperacion };
    const consultacxp = new Consulta('TcarOperacionCxP', 'Y', '', mfiltroscxp, null);
    this.addConsultaCatalogos('CXP', consultacxp, null, this.llenaCxp, null, this.componentehijo);

    this.ejecutarConsultaCatalogos();
  }

  public llenaCxp(pLista: any, pListaResp, campopk = '', agregaRegistroVacio = true, componentehijo = null): any {
    if (pListaResp !== null && pListaResp.length > 0) {
      componentehijo.rqMantenimiento.monto = pListaResp[0].saldo;
    }
  }
}
