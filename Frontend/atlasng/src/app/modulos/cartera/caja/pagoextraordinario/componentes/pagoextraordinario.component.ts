...
@Component({
  selector: 'app-pago',
  templateUrl: 'pagoextraordinario.html'
})
export class PagoExtraordinarioComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  grabar(): void {
    ...
    if (this.rqMantenimiento.mantieneplazo === "0") {
      this.rqMantenimiento.valorcuota = this.mcampos.valorcuota;
    }

    const fechacont = super.integerToFormatoFecha(this.dtoServicios.mradicacion.fcontable);
    const fecha = new Date(new Date(fechacont).getFullYear(), Number(fechacont.toString().substring(3, 5)), 0);
    this.rqMantenimiento.mdatos.finiciopagos = super.fechaToInteger(fecha);
  ...
  fijarLovOperacionSelec(reg: any): void {
    this.diapago = reg.registro.diapago;
  ...
  public consultarDatosOperacion() {
    ...
    rqConsulta.CODIGOCONSULTA = 'SALDOALAFECHACARTERA';
    ...
    this.dtoServicios.ejecutarConsultaRest(rqConsulta)
      .subscribe(
        resp => {
          ...
          this.mcampos.valorcuota = resp.OPERACION[0]['valorcuota'];
        },
        error => {
