...
@Component({
  selector: 'app-seguros-descuentos',
  templateUrl: 'segurosDescuentos.html'
})

export class SegurosDescuentosComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  imprimir(): void {
    if (!this.estaVacio(this.mcampos.particion)) {
      ...
      this.jasper.nombreArchivo = 'SegurosDescuentos_' + this.mcampos.particion;
      this.jasper.parametros['@particion'] = this.mcampos.particion;
      this.jasper.parametros['archivoReporteUrl'] = '/CesantiaReportes/Seguros/rptSgsDescuentos';
