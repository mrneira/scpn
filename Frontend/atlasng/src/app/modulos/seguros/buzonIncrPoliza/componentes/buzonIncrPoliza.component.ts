...
@Component({
  selector: 'app-buzonIncrPoliza',
  templateUrl: 'buzonIncrPoliza.html'
})
export class BuzonIncrPolizaComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  constructor(router: Router, dtoServicios: DtoServicios, private confirmationService: ConfirmationService) {
    super(router, dtoServicios, 'TsgsPoliza', 'BUZONPOLIZASINCREMENTO', false, true);
  ...
  public fijarFiltrosConsulta() {
    ...
    this.mfiltrosesp.coperacioncartera = 'in (select coperacioncartera from tsgsseguro where incremento = \'TRUE\')';
    this.mfiltros.secuencia = 1;
    this.mfiltros.renovacion = null;
  }
  ...
  validaRegistros() {
    ...
    if (this.selectedRegistros != null && this.selectedRegistros.length > 0) {
      this.rqMantenimiento.mdatos.accion = "incremento";
      return true;
    }
    return false;
  }
