...
namespace activosfijos.comp.mantenimiento.activosfijos.depreciacion {
    ...
    public class Contabilizar : ComponenteMantenimiento {
        ...
        public override void Ejecutar(RqMantenimiento rqmantenimiento) {
            ...
            tacfproductocodificado codificados = new tacfproductocodificado();
            List<tacfproductocodificado> lcodificados = new List<tacfproductocodificado>();

            tacfproducto producto = new tacfproducto();
            List<tacfproducto> lproducto = new List<tacfproducto>();

            foreach (tacfhistorialdepreciacion obj in lista)
            {
                codificados = TacfProductoCodificadoDal.FindxCproducto(obj.cproducto, obj.cbarras);
                codificados.Esnuevo = false;
                codificados.Actualizar = true;
                codificados.valorlibros =  obj.valordepreciable.Value;
                codificados.valorresidual = obj.valorresidual;
                lcodificados.Add(codificados);

                producto = TacfProductoDal.Find(obj.cproducto);
                producto.Esnuevo = false;
                producto.Actualizar = true;
                producto.valorlibros =  obj.valordepreciable.Value; 
                producto.valorresidual = obj.valorresidual;
                lproducto.Add(producto);

            }
            rqmantenimiento.AdicionarTabla("tacfproductocodificado", lcodificados, false);
            rqmantenimiento.AdicionarTabla("tacfproducto", lcodificados, false);
        ...
        public static tconcomprobante CompletarComprobante(RqMantenimiento rqmantenimiento, tacfdepreciacion depreciacion) {
            ...
            tconcomprobante comprobante = TconComprobanteDal.CrearComprobante(ccomprobante,rqmantenimiento.Fconatable,Constantes.GetParticion((int)rqmantenimiento.Fconatable),
                rqmantenimiento.Ccompania, 0, null, null, null, rqmantenimiento.Freal, rqmantenimiento.Fproceso, depreciacion.comentario, true, true, false, false, false, false, false,
                null, 3, rqmantenimiento.Cagencia,rqmantenimiento.Csucursal, rqmantenimiento.Ctransaccion,rqmantenimiento.Cmodulo, 1003, "DIAGEN", rqmantenimiento.Cagencia, rqmantenimiento.Csucursal, 0, numerocomprobantecesantia,
