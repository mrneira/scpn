...
namespace bancaenlinea.comp.mantenimiento {
    ...
    public class PoliticaSeguridad : ComponenteMantenimiento {
        ...
        public override void Ejecutar(RqMantenimiento rqmantenimiento) {
            ...
            tbanusuarios det = null;
            if (rqmantenimiento.GetTabla("CAMBIOCONTRASENIA") != null) {
                det = (tbanusuarios)rqmantenimiento.GetTabla("CAMBIOCONTRASENIA").Lregistros.ElementAt(0);
                if (det != null) {
                    if (det.Esnuevo) {
                        det = TbanUsuariosDal.Find(cusuario);
                ...
                det = TbanUsuariosDal.Find(cusuario);
                EntityHelper.SetActualizar(det);
                //this.ValidaPassword(rqmantenimiento, det);
                this.Validate(tsegPolitica, valor);
                // Valida que no se repitan los ultimos n passwords
                PoliticaSeguridad.ValidateNotrepeated(det, tsegPolitica);
        ...
        /// <param name="det">Objeto tbanusuarios</param>
        private void ValidaPassword(RqMantenimiento rqmantenimiento, tbanusuarios det) {
        ...
        /// </summary>
        /// <param name="tsegPolitica">Objeto que contiene la politica de seguridad</param>
        public static void ValidateNotrepeated(tbanusuarios usuariodetalle, tsegpolitica tsegPolitica) {
            if (tsegPolitica.repeticiones == null || tsegPolitica.repeticiones.Equals(0)) {
                return;
            }
            IList<Dictionary<string, object>> ldata = TbanUsuariosDal.FindPasswords(usuariodetalle.cusuario, tsegPolitica.repeticiones ?? 0);
