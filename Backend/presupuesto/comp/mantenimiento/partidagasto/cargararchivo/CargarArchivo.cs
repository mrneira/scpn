...
namespace presupuesto.comp.mantenimiento.partidagasto.cargararchivo
{
        ...
        public override void Ejecutar(RqMantenimiento rqmantenimiento)
        {
            if (rqmantenimiento.Mdatos["cargaarchivo"].ToString() == "upload")
            {
                ...
                while (true) {
                    ...
                    string cpartidagasto = "";
                    string padre = "";
                    int nivel;
                    int movimiento;
                    string nombre = "";
                    int aniofiscal;
                    decimal vasignacioninicial = 0;

                    string lstrMensaje = "";
                    string lestado = "Ok";
                    cpartidagasto = lclsValidar.clsValidarStrLenght(lclsValidar.asignarObjeto(worksheet.Range["A" + lstrFilas].Value2), 10, ref lstrMensaje, "partida");


                    if (cpartidagasto.Equals("")) {
                        break;
                    }
                    if (TpptPartidaGastoDal.FindClasificador(cpartidagasto) == null) { 
                    ...
                    vasignacioninicial  = lclsValidar.clsValidarDecimal(lclsValidar.asignarObjeto(worksheet.Range["G" + lstrFilas].Value2), ref lstrMensaje, "asignacion_inicial");

                    if (TpptPartidaGastoDal.FindListaAniofiscal(aniofiscal).Count > 0) {
                ...
                rqmantenimiento.Response["lregistros"] = lpartidagasto;
            }
            else
            if (rqmantenimiento.Mdatos["cargaarchivo"].ToString() == "read")
            {
                List<tpptpartidagasto> lpartida = JsonConvert.DeserializeObject<List<tpptpartidagasto>>(rqmantenimiento.Mdatos["lregistros"].ToString());
                foreach (tpptpartidagasto item in lpartida) {
                    item.Esnuevo = true;
                    ...
                    item.vmodificado = 0;
                    item.vcodificado = item.vasignacioninicial;
                    item.vcomprometido = 0;
                    item.vdevengado = 0;
                    item.vpagado = 0;
                    item.vsaldoporcomprometer = 0;
                    item.vsaldopordevengar = 0;
                    item.vsaldoporpagar = 0;
