...
{
    public class VarRentaVariable : ComponenteConsulta
    {
        public override void Ejecutar(RqConsulta rqconsulta)
        {
            //RENTA VARIABLE- TRABAJA CON PRECIOS DE CIERRE
            ...
            decimal prnper = TinvParametrosDal.GetValorNumerico("PROBABILIDADNPERDIDA-VAR", rqconsulta.Ccompania);

            //DISTRIBUCIÓN NORMAL
            decimal dis = TinvParametrosDal.GetValorNumerico("DISTRIBUCIONNORMALIND-VAR", rqconsulta.Ccompania);
            ...
            System.TimeSpan dif = ffin - finicio;
            int contemi = 0;
            ...
            foreach (tgencatalogodetalle emi in Emisores)
            {
                ...
                for (int n = 0; n <= dif.Days; n++)
                {
                    ...
                    double precio = varDal.precioCierreVarValoracion(emi.cdetalle, fvaloracion);
                    //pr[n] = precio;
                ...
                if (listaigual(pr)) {
                    throw new AtlasException("INV-011", "EL EMISOR {0} NO TIENE UNA VARIACIÓN DE VALORES EN LAS FECHAS {1} A {2}", nombre,finicio.ToString("yyyy/MM/dd"), ffin.ToString("yyyy/MM/dd"));
               ...
               // listadestardar[contemi]=rend;
                ...
                double desvestun =(double)decimal.Round((decimal)desv.valor, 16, MidpointRounding.AwayFromZero);
           ...
           // var dvarsincorrelacion = JsonConvert.SerializeObject(varportsncorr);
            ...
            general.reporte.GenerarArchivo.generarArchivo(tipoarchivo, "/CesantiaReportes/Inversiones/", "rptInvVarRentaVariable", "c:\\tmp\\", "rptInvVarRentaVariable", parametrosPdf, true, out archivo);
            rqconsulta.Response["ARCHIVOVAR"] = archivo;
            
        }
        static bool listaigual(double[] valores) {
            decimal prom = 0,sum=0;
            int nrep = 0;
            
            foreach (double valor in valores) {
                sum = sum + (decimal)valor;
            }
            prom = sum / (valores.Length);
            foreach (double valor in valores)
            {
                if (valor == (double)prom) {
                    nrep++;
                }
            }
            return (nrep == (valores.Length)) ? true : false;
