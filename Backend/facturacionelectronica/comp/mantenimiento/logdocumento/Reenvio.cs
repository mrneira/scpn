...
namespace facturacionelectronica.comp.mantenimiento.logdocumento
{
    ...
    public class Reenvio : ComponenteMantenimiento
    {
        /// <summary>
        ...
        {
            if (rm.GetTabla("LOGDOCUMENTOS") == null || !rm.GetTabla("LOGDOCUMENTOS").Lregistros.Any())
            {
                return;
            }
            List<tcellogdocumentos> ldatos = rm.GetTabla("LOGDOCUMENTOS").Lregistros.Cast<tcellogdocumentos>().ToList();
            foreach (tcellogdocumentos obj in ldatos)
            {
                if (obj.Esnuevo)
                {
                    string tipoDocumento = obj.tipodocumento;
                    string claveAcceso = obj.clavedeacceso;
                    string numeroDocumento = obj.numerodocumento;
                    EntidadComprobante entidad = new EntidadComprobante();
                    Thread.CurrentThread.CurrentCulture = new CultureInfo("es-PE");
                    switch (tipoDocumento)
                    {
                        case "FA":
                            tconcuentaporcobrar cxc = TconCuentaPorCobrarDal.FindXNumeroDocumento(obj.numerodocumento);

                            #region Armar entidad
                        ...
                        case "CR":
                            tconcuentaporpagar cxp = TconCuentaporpagarDal.FindXNumeroDocumento(obj.numerodocumento);
                            ...
                            entidad.ClaveAcceso = claveAcceso;
                            entidad.FechaEmisionDocumento = rm.Freal.ToString("dd/MM/yyyy");
                            ...
                            int indice = 2;
                            foreach (tconretencionfuente objRetencion in ldetalle)
                            {
                                entidad.DetalleImpuestosRetencion[indice] = new EntidadImpuestoRetencion()
                                {
                                    Codigo = 1,
                                    CodigoRetencion = objRetencion.codigosri,
                                    BaseImponible = objRetencion.baseimpair.Value,
                                    PorcentajeRetenido = objRetencion.porcentaje.Value,
                                    ValorRetenido = objRetencion.valretair.Value,
                                    CodigoDocumentoSustento = TgenCatalogoDetalleDal.Find(cxp.tcomprobanteccatalogo.Value, cxp.tcomprobantecdetalle).clegal,
                                    DocumentoSustento = cxp.numdocumentosustento.Replace("-", ""),
                                    FechaDocumentoSustento = cxp.ffacturasustento.Value.ToString("dd/MM/yyyy")
                                };
                    ...
                    TcelLogDocumentosDal.UpdateDocumentoReenviado(obj.clog);
                }
            }
        }
